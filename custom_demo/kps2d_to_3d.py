...
def get_parser():
    ...
    parser.add_argument('--lefthand3d-lifter-config', default='configs/hand/3d_kpt_sview_rgb_img/simplebaseline3d/simplebaseline3d_h3wb_left_hand.py')
    parser.add_argument('--lefthand3d-lifter-checkpoint', default='work_dirs/simplebaseline3d_h3wb_left_hand/best_MPJPE_epoch_200.pth')
    
    parser.add_argument('--righthand3d-lifter-config', default='configs/hand/3d_kpt_sview_rgb_img/simplebaseline3d/simplebaseline3d_h3wb_right_hand.py')
    parser.add_argument('--righthand3d-lifter-checkpoint', default='work_dirs/simplebaseline3d_h3wb_right_hand/best_MPJPE_epoch_200.pth')
    
    parser.add_argument('--leftfoot3d-lifter-config', default='configs/foot/3d_kpt_sview_rgb_img/pose_lift/h3wb/simplebaseline3d_h3wb_left_foot.py')
...
def main(args):
    ...
    with open(args.pkl_path, 'rb') as fin:
        video_output_list = pickle.load(fin)
    ...
    assert len(video) == len(video_output_list)
    ...
    for frame_id, cur_frame in enumerate(mmcv.track_iter_progress(video)):
        ...
        single_frame_dict = video_output_list[frame_id]
        
        # postprocess hands, using pre and post frame, using another hand
        for tid, person in single_frame_dict.items():
            postprocess_hands_inplace(person, pre_frame_id, post_frame_id, video_output_list)    
        ...
        wholebody_wo_face_lift_results = merge_kpts_according_trackid([body_lift_results, lhand_lift_results, 
                                                                       rhand_lift_results, lfoot_lift_results, rfoot_lift_results])
        
        # post process for better visualize
        for person in wholebody_wo_face_lift_results:
        ...
        if len(wholebody_wo_face_lift_results) == 0:
            continue
        
        img_vis = vis_3d_pose_result(
            body_lift_model,
            result=wholebody_wo_face_lift_results,
            img=cur_frame,
